...
const About = () => {
  ...
  const stats = [
    ...
    { number: 2, label: 'Years Experience', suffix: '+', icon: '⏱' },
  ...
  const infoCards = [
    {
      ...
      text: "Hi, I'm Rian Hasan Siam, a passionate Software Engineering student at Daffodil International University. I specialize in full-stack web development with a focus on creating innovative, scalable solutions."
    },
    {
      title: 'What I Do',
      icon: '🎯',
      color: 'from-purple-500/20 to-pink-500/20',
      text: "I build modern web applications using cutting-edge technologies like React, Next.js, and Node.js. From frontend interfaces to backend APIs, I enjoy the entire development process."
    },
    {
      title: 'What Drives Me',
      icon: '💜',
      color: 'from-pink-500/20 to-rose-500/20',
      text: "My passion for technology and continuous learning drives me to stay updated with the latest trends. I believe in writing clean, maintainable code and creating user experiences that make a difference."
    },
  ]
          ...
          stats.forEach((stat, index) => {
            gsap.to({}, {
              ...
              onUpdate: function() {
                ...
                setCounters(prev => {
                  const next = [...prev]
                  next[index] = Math.floor(stat.number * progress)
                  return next
                })
              },
              ease: 'power2.out',
      ...
      <div className="container-custom relative z-10 px-4 sm:px-6 lg:px-8">
          ...
          <div>
            ...
            <div ref={statsRef} className="grid grid-cols-2 gap-4">
              {stats.map((stat, i) => (
                <div
                  key={i}
                  className="glass-card p-5 text-center group hover:border-purple-500/20 transition-all duration-500 hover:-translate-y-1"
                >
                  <span className="text-2xl mb-2 block">{stat.icon}</span>
                  <div className="text-3xl font-bold gradient-text counter-value mb-1">
          ...
          <div className="space-y-6">
            {infoCards.map((card, i) => (
              <div
                ...
                className="glass-card p-7 group hover:border-purple-500/20 transition-all duration-500"
              >
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: '💻', title: 'Programming', desc: 'Solving complex problems' },
              { icon: '🎮', title: 'Gaming', desc: 'Strategic & open world' },
              { icon: '📺', title: 'Sports', desc: 'Watching live matches' },
              { icon: '⚽', title: 'Football', desc: 'The beautiful game' },
            ].map((item, i) => (
